...
public class FanSystem : MonoBehaviour
{
    ...
    public Vector3 fanModelRotation = new Vector3(-90f, 0f, 0f);
    public Vector3 fanModelScale = new Vector3(7f, 7f, 7f);
    [Range(3, 20)]
    ...
    void Start()
    {
        if (fanAnimator != null)
        {
            GameObject fanObj = Instantiate(fanAnimator.gameObject, transform);
            fanObj.transform.localPosition = Vector3.zero;
            fanObj.transform.localRotation = Quaternion.Euler(fanModelRotation);
            fanObj.transform.localScale = fanModelScale;
            fanAnimator = fanObj.GetComponent<Animator>();
        }

        CreatePrototypeFan();

        // 실제 부채 모델이 있으면 큐브 리브 메시는 숨김
        if (fanAnimator != null)
        {
            foreach (var rib in ribs)
            {
                foreach (var mr in rib.GetComponentsInChildren<MeshRenderer>())
                    mr.enabled = false;
            }
        }
        ...
        // Toffee's Wind Trail visual effect attachment
        // 프리팹에 수동으로 붙여둔 WindTrailEffect가 없다면 자동 생성 (미리 세팅할 수 있도록 배려)
        if (GetComponentInChildren<WindTrailEffect>() == null)
        {
            WindTrailEffect.AttachTo(this);
        }
    }
    ...
    void AnimateFan()
    {
        ...
        if (fanAnimator != null)
        {
            // FanAnim: normalizedTime 0=열림, 1=닫힘 → currentOpenAmount 반전
            fanAnimator.Play("FanAnim", 0, 1f - currentOpenAmount);
            fanAnimator.speed = 0f;
        }

        // 충돌 판정용 큐브 리브는 항상 회전 유지
    ...
    // --- 부채 스윙 의도 판별 로직 ---
    public bool IsFanningMotion
    {
        get
        {
            if (currentVelocity.magnitude < fanningThreshold) return false;
            // 이동 방향과 부채의 법선 벡터가 비슷하면(수평) 부치기
            float dot = Mathf.Abs(Vector3.Dot(currentVelocity.normalized, FanNormal));
            return dot > 0.6f;
        }
    }

    public bool IsSlashingMotion
    {
        get
        {
            if (currentVelocity.magnitude < fanningThreshold) return false;
            // 이동 방향과 부채의 법선 벡터가 직교에 가까우면 베기
            float dot = Mathf.Abs(Vector3.Dot(currentVelocity.normalized, FanNormal));
            return dot <= 0.6f;
        }
    }
