...
public class Note : MonoBehaviour
{
    ...
    public void SetupVisuals()
    {
        ...
        if (wallIndicator) wallIndicator.SetActive(false);

        BoxCollider boxCollider = GetComponent<BoxCollider>();
        if (boxCollider != null)
        {
            if (type == NoteType.Wall) boxCollider.size = Vector3.one;
            else boxCollider.size = new Vector3(1.5f, 1.5f, 1.5f);
        }
        ...
        if (type == NoteType.Wall)
        {
            transform.rotation = Quaternion.identity;
            Vector3 wallScale = targetDirection;
            if (wallScale.sqrMagnitude < 0.1f) wallScale = new Vector3(0.5f, 3f, 15f); // default scale if not set
            transform.localScale = wallScale;
            if (wallIndicator) { wallIndicator.SetActive(true); wallIndicator.transform.localRotation = Quaternion.identity; }
        }
        else
        {
            ...
            switch (type)
            {
                ...
                case NoteType.Boss: 
                    ...
                    { 
                        ...
                        bossIndicator.transform.localRotation = Quaternion.identity; 
                        bossIndicator.transform.localScale = Vector3.one; // 보스 크기 리셋
                    } 
                    break;
            }
        }
    ...
    void Update()
    {
        ...
        float currentTime = (float)(GameManager.Instance != null ? GameManager.Instance.EffectiveDspTime : AudioSettings.dspTime);
        float spawnTime = hitTime - travelTime;
        float progress = (currentTime - spawnTime) / travelTime;

        float currentZ = Mathf.LerpUnclamped(spawnZ, hitZ, progress);

        // 🌟 [WALL OFFSET FIX] 벽은 앞뒤(Z축)로 길기 때문에 중심(Center)을 뒤로 밀어줍니다.
        // 이렇게 해야 '벽의 앞면(Front)'이 일반 노트와 완전히 동일한 Z 위치에서 시작하고, 동일한 타이밍에 유저에게 도달합니다.
        if (type == NoteType.Wall)
        {
            currentZ += transform.localScale.z / 2f;
        }
        ...
        if (type == NoteType.Wall)
        {
            // 🌟 [WALL DESPAWN FIX] 벽은 길이가 기므로, 꼬리(뒷면)까지 유저를 완전히 통과한 후에 삭제해야 합니다.
    ...
    public void Deactivate()
    {
        ...
        {
            NotePoolManager.Instance.ReturnNote(gameObject, type);
        }
        else
        {
            Destroy(gameObject);
    ...
    private void OnTriggerStay(Collider foreign)
    {
        ...
        // 🌟 [HIT ZONE GUARD] Update()의 스윕 판정과 동일하게, 일반 노트는 판정선 1.2m 이내에서만 처리합니다.
        // 이 체크가 없으면 노트가 아직 멀리 있을 때도 BoxCollider 접촉으로 판정이 터질 수 있습니다.
        if (type != NoteType.Wall && Mathf.Abs(transform.position.z - hitZ) >= 1.2f) return;

        if (type == NoteType.Wall)
        {
            if (foreign.CompareTag("PlayerHead"))
            {
                isMissed = true;
                if (GameManager.Instance != null) GameManager.Instance.NoteMissed();
                // 벽은 부딪혀도 바로 사라지지 않고 끝까지 지나가도록 Deactivate() 제거
            }
            return;
        }
    ...
    void CheckHitSuccessWithDirection(FanSystem fan, Vector3 moveDir)
    {
        ...
        if (isCorrectAction)
        {
            ...
            {
                if (GameManager.Instance != null && GameManager.Instance.customHitEffectPrefab != null)
                {
                    GameObject fx = Instantiate(GameManager.Instance.customHitEffectPrefab, transform.position, Quaternion.identity);
                    Destroy(fx, 2f);
                }
                else
                {
                    InkSplashEffect.Spawn(transform.position, type, judgment);
                }
            }
